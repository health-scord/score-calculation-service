  ...
  async getData(account) {
    try {
      let userData = await Promise.all([
        getHeartData(account),
        getFitnessActivityData(account),
        getSleepData(account)
      ]);
...
let getHeartData = async account => {
  let token = account.devices[0].accessToken;

  let response = await this.fitbitClient.get(
...
let getFitnessActivityData = async account => {
  let token = account.devices[0].accessToken;
...
let getSleepData = async account => {
  let token = account.devices[0].accessToken;

  let response = await this.fitbitClient.get(
    "/sleep/date/2019-06-21.json",
    token
  );
