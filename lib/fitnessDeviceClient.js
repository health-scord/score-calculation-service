...
class fitnessDeviceClient {
  ...
  async getData(account) {
    try {
      ...
      let averageDailyActiveMinutes = calculateAverageDailyActivity(
        activityData["activities-tracker-minutesVeryActive"]
      );
      let restingHeartRate = calculateRestingHeartRate(
        heartRateData["activities-heart"]
      );

      console.log(`average daily sleep: ${averageDailySleep}`);
      console.log(`average daily active minutes: ${averageDailyActiveMinutes}`);
...
const calculateAverageDailySleep = sleepData => {
  ...
  let averageDailySleep = totalSleep / uniqueDates.length;
...
const calculateAverageDailyActivity = activityData => {
  ...
  let averageDailyActiveMinutes = totalActiveMinutes / activityData.length;

  return averageDailyActiveMinutes;
};
...
const calculateRestingHeartRate = heartData => {
  ...
  return heartRateSum / releventData.length;
};
